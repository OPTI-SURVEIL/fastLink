...
emlinklog_b <- function(patterns, nobs.a, nobs.b,
  ...
  if(is.null(varnames)) varnames = 1:(ncol(patterns)-1)
    ...
    #pat[is.na(pat)] <- -1
    pat <- replace(pat, TRUE, lapply(pat, factor,exclude = c()))
    ...
    dropcols = grepl('NA:|:.*NA$',colnames(mm.m)); mm.m = mm.m[,!dropcols]
    dropcols = grepl('NA:|:.*NA$',colnames(mm.u)); mm.u = mm.u[,!dropcols]
    ...
    while (abs(delta) >= tol) {
        ...
        c <- 1e-06
        matches <- glm(count ~ ., data = data.frame(count = ((zeta.j * n.j) + c), mm.m),
                       family = "quasipoisson")

        non.matches <- glm(count ~ ., data = data.frame(count = ((1 - zeta.j) * n.j + c), mm.u),
                           family = "quasipoisson")
