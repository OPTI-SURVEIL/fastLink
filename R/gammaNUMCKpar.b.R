...
#'
#' @usage gammaNUMCKpar.b(matAp, matBp, blocklist, dedupe.blocks, n.cores, cut.a, cut.p)
#'
...
#' @return \code{gammaNUMCKpar.b} returns a list of lists (for each block) with the indices corresponding to each
...
gammaNUMCKpar.b <- function(matAp, matBp, blocklist, n.cores = NULL, cut.a = 1, cut.p = 2, dedupe.blocks) {
  varnm = names(matAp)
  ...
  registerDoSNOW(cl1)
  clusterExport(cl1, c('matrix.1','matrix.2','blocklist','dedupe.blocks','cut.a','cut.p'),
                envir = environment())
  ...
  difference <- function(m, y, cut,identical) {
    ...
    if(identical){
      ...
      t[lower.tri(t)] = res
      diag(t) = 0
      diag(t)[x == end.points[2]] <- cut[2] + 1 #keep out NAs
    }else{
  ...
  if(sum(inpar)>0){
    ...
    out[inpar] = foreach(i = 1:sum(inpar), 
      ...
      if(ddp & length(u.values.1) == 1){
        ...
        temp.f <- difference(list(u.values.1,0), list(u.values.2,0), c(cut.a,cut.p), identical = ddp)
        ...
        values2store.1 = unique(c(n.values.2[,1],n.values.1[,1]))
        values2store.2 = unique(c(n.values.2[,2],n.values.1[,2]))
        ...
        final.list1 <- lapply(matches.1, function(x){
          ht1 <- ht.1[[as.character(x[[1]])]]; ht2 <- ht.2[[as.character(x[[2]])]]
          list(ht1, ht2)
        })
      }
      ...
      out = list(matches2 = final.list2, matches1 = final.list1, nas = na.list, .identical = .identical)
  ...
  if(sum(!inpar) > 0){
    ...
    for(j in (sum(inpar)+1):length(blocklist)){
      ...
      temp.f <- foreach(i = 1:nrow(do), .packages = c("Matrix",'fastLink','RcppAlgos')) %dopar% {
        ...
        difference(temp[[r1]], temp[[r2]], c(cut.a,cut.p), identical = r1==r2 & ddp)
      }
      
      indexes.2 <- do.call('rbind', lapply(temp.f,'[[',1))
      indexes.1 <- do.call('rbind', lapply(temp.f,'[[',2))
      ...
      final.list1 <- foreach(i = 1:length(matches.1)) %dopar% {
        ht1 <- ht.1[[as.character(matches.1[[i]][[1]])]]; ht2 <- ht.2[[as.character(matches.1[[i]][[2]])]]
        list(ht1, ht2)
      }
      ...
      out[!inpar][[j-sum(inpar)]][["matches1"]] <- final.list1
      out[!inpar][[j-sum(inpar)]][["nas"]] <- na.list
      out[!inpar][[j-sum(inpar)]][[".identical"]] <- .identical
      class(out[!inpar][[j-sum(inpar)]]) <- c("fastLink", "gammaNUMCKpar")
