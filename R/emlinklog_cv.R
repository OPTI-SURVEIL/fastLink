#' emlinklog_cv
#'
#' Expectation-Maximization with internal cross validation for Record Linkage 
#' allowing for dependencies across linkage fields
#'
#' @usage emlinklog_cv(patterns, nobs.a, nobs.b, p.m, p.gamma.j.m, p.gamma.j.u,
#' iter.max, tol, varnames, k)
#'
...
#' @param k Fold of internal cross validation. Defaults to 50.
#'
...
#'
#' ## Run EM
#' em.log <- emlinklog_cv(tc, nobs.a = nrow(dfA), nobs.b = nrow(dfB))
#' }
#'
#' @export
#' @importFrom gtools rdirichlet
#' @importFrom stats glm model.matrix
emlinklog_cv <- function(patterns, nobs.a, nobs.b,
                      p.m = NULL, p.gamma.j.m = NULL, p.gamma.j.u = NULL, iter.max = 5000, tol = 1e-5, varnames = NULL, k = 50) {
    ...
    #generating CV fold data
    
    folds = vector('list', length = k)
    cnts = patterns[,'counts']
    nget = round(sum(cnts)/ k, 0)
    
    for(i in 1:(k-1)){
      nsmp = min(nget,sum(cnts))
      p.obs = cnts / sum(cnts)
      samp = rmultinom(1,nsmp,p.obs)
      while(any(cnts - samp < 0)) samp = rmultinom(1,nsmp,p.obs)
      folds[[i]] = samp
      cnts = cnts - samp
    }
    folds[[k]] = cnts
    ...
    ## Patterns counts:
    n.j <- do.call(cbind,lapply(folds,as.matrix))  # Counts
    ...
    LL_1 = dmultinom(patterns[,'counts'] * zeta.j, prob = p.gamma.j.m, log = T) + dmultinom(patterns[,'counts'] * (1-zeta.j), prob = p.gamma.j.m, log = T)
    ...
    while (abs(delta) >= tol) {
      ...
      ## M-step :
      ## --------
      num.prod <- apply(n.j, 2, '*', zeta.j)
      p.m <- colSums(num.prod)/colSums(n.j)
      p.u <- 1 - p.m
      ...
      c <- 1e-06
      matches <- lapply(1:k, function(i) glm(count ~ ., data = data.frame(count = ((zeta.j * n.j[,i]) + c), factors),
                                             family = "quasipoisson"))
      
      non.matches <- lapply(1:k, function(i) glm(count ~ .*., data = data.frame(count = ((1 - zeta.j) * n.j[,i] + c), factors),
                                                 family = "quasipoisson"))
      ...
      p.gamma.j.m = lapply(1:k, function(i) as.matrix(g(matches[[i]])))
      p.gamma.j.u = lapply(1:k, function(i) as.matrix(g(non.matches[[i]])))
