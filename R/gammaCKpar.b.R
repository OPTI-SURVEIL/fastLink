#' gammaCKpar.b
#'
#' Field comparisons for string variables for blocked data. Three possible agreement patterns are considered:
...
#'
#' @usage gammaCKpar.b(matAp, matBp, blocklist, dedupe.blocks, n.cores, cut.a, cut.p, transform, transform.args, method, method.args, w)
#'
...
#' @param cut.p Lower bound for partial match. Default is 0.88
...
gammaCKpar.b <- function(matAp, matBp, blocklist, n.cores = NULL, cut.a = 0.92, cut.p = 0.88, dedupe.blocks,
                         transform = NULL, transform.args, method = 'jw', method.args, w = 0.1) {
  varnm = names(matAp)
  ...
  if(sum(inpar)>0){
    ...
    out[inpar] = foreach(i = 1:sum(inpar), 
      ...
      if(ddp & length(u.values.1) == 1){
        ...
        temp.f <- stringvec(list(temp.1,0), list(temp.2,0), c(cut.a,cut.p), identical = ddp)
      ...
      out = list(matches2 = final.list2, matches1 = final.list1, 
                 nas = na.list, .identical = .identical)
  ...
  if(sum(!inpar) > 0){
    ...
    for(j in (sum(inpar)+1):length(blocklist)){
      ...
      temp.f <- foreach(i = 1:nrow(do), .packages = c("Matrix",'fastLink','RcppAlgos',pkgs), .options.snow = opts2) %dopar% {
        ...
        stringvec(temp[[r1]], temp[[r2]], c(cut.a,cut.p), identical = r1==r2 & ddp)
      }
      close(pb2)
      ...
      class(out[!inpar][[j-sum(inpar)]]) <- c("fastLink", "gammaCKpar")
