        ...
        if(identical == true){
          ...
          for(i = 0; i < temp0.n_elem; i++){
            for(j = 0; j < temp1.n_elem; j++){
              index_out(rowcount,0) = min(temp0[i], temp1[j]);
              index_out(rowcount,1) = max(temp0[i], temp1[j]);
              rowcount++;
            }
          }
        }
...
std::vector<arma::vec> indexing_na(const std::vector<arma::vec> s,
  ...
  if(dedupe == true){
    arma::uvec s0_l2 = s0 <= min(l2,l4-1); //double constraint for upper triangular dedup matrix
  }else{
    arma::uvec s0_l2 = s0 <= l2;
  }
